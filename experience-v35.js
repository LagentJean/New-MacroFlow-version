(() => {
  ...
  function boot() {
    refresh();
    window.addEventListener('macroflow:general-state-rendered', (event) => { lastGeneral = event.detail; updateFlowCore(lastGeneral); });
    window.addEventListener('macroflow:home-rendered', () => requestAnimationFrame(() => updateFlowCore(lastGeneral)));
    window.addEventListener('macroflow:training-state-rendered', (event) => { lastTraining = event.detail; requestAnimationFrame(() => installProgramSpotlight(lastTraining)); });
    observer = new MutationObserver(() => {
      simplifyOnboarding();
      localizeInterface();
      installIconSystem();
      if (lastTraining) compactProgram(lastTraining.program);
    });
